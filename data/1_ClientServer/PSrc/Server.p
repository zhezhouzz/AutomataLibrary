...
type tWithDrawReq = (source: machine, dest: server, accountId: int, amount: int, rId:int);
...
type tWithDrawResp = (dest: server, status: bool, accountId: int, balance: int, rId: int);
  ...
  state WaitForWithdrawRequests {
    on eWithDrawReq do (wReq: tWithDrawReq) {
      ...
      if(currentBalance - wReq.amount >= 10)
      {
        ...
        response = (dest = wReq.dest, status = true, accountId = wReq.accountId, balance = currentBalance - wReq.amount, rId = wReq.rId);
      }
      else // not enough money after withdraw
      {
        response = (dest = wReq.dest, status = false, accountId = wReq.accountId, balance = currentBalance, rId = wReq.rId);
      }
