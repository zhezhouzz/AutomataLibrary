    ...
    on eWithDrawReq do (wReq: tWithDrawReq) {
      ...
      if(balance[wReq.accountId] - wReq.amount > 10) /* hint: bug */
      {
        ...
        send wReq.source, eWithDrawResp,
          (dest = wReq.dest, status = true, accountId = wReq.accountId, balance = balance[wReq.accountId], rId = wReq.rId);
      }
      else
      {
        send wReq.source, eWithDrawResp,
          (dest = wReq.dest, status = false, accountId = wReq.accountId, balance = balance[wReq.accountId], rId = wReq.rId);
      }
    }
  }
}